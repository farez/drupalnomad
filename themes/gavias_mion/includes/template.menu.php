...
function gavias_mion_preprocess_menu__main(&$variables) {
  $variables['attributes']['class'][] = 'clearfix sm sm-blue';

  foreach ($variables['items'] as &$item) {
   $menu_link_attributes = _gavias_mion_attributes_get_attributes($item['original_link']);
      ...
      if (count($menu_link_attributes)) {
         ...
         if(isset($attributes['gva_layout']) && $attributes['gva_layout']=='menu-block'){
            $item['gva_block_content'] = gavias_mion_render_block($attributes['gva_block']);
         }
     }
   }
}

function _gavias_mion_attributes_get_attributes(MenuLinkInterface $menu_link_content_plugin) {
  $attributes = [];
  try {
